...
@implementation CompanyFileCell
...
- (void)setDocumentModel:(DocumentMergeModel *)DocumentModel
{
    ...
    [self.markImageView sd_setImageWithURL:[NSURL URLWithString:[NSString replaceString:self.DocumentModel.thumbnailUrl Withstr1:@"100" str2:@"100" str3:@"c"]] placeholderImage:[DocumentOtherMethod getDocumentThumImageWithfileName:_DocumentModel.documentName]];
    ...
    if (self.type == 1) {
        self.createTimeLabel.text = _DocumentModel.Catalog;
...
- (void)setupContentView
{
    ...
    //___
    self.createTimeLabel.frame = CGRectMake(self.fileNameLabel.left, self.fileNameLabel.bottom, fileNameLabelW, 10);
...
- (void)layoutSubviews
{
    ...
    if (self.height <= 50) {
        ...
        self.fileNameLabel.numberOfLines = 0;
        self.createTimeLabel.frame = CGRectMake(self.fileNameLabel.left, self.fileNameLabel.bottom + 5, fileNameLabelW, 10);
        self.separatorLine.frame = CGRectMake(0, self.height - 0.5, kScreen_Width, 0.5);
    }
    if (self.height > 50 && self.height < 90) {
        self.fileNameLabel.frame = CGRectMake(fileNameLabelX, fileNameLabelY, fileNameLabelW, self.height - 35);
        self.fileNameLabel.numberOfLines = 0;
        self.createTimeLabel.frame = CGRectMake(self.fileNameLabel.left, self.fileNameLabel.bottom + 5, fileNameLabelW, 10);
        self.separatorLine.frame = CGRectMake(0, self.height - 0.5, kScreen_Width, 0.5);
    ...
    [vc setFileOperateBlock:^(NSInteger filetype, NSInteger tag) {
        ...
        if (filetype == 0) {
            ...
            if (tag == 1) {
                ...
                moveFolderVC.departmentId = @""; //公司的话，这个值一直为@""
