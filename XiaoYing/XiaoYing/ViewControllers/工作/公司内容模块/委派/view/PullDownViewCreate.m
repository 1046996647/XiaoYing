...
@implementation PullDownViewCreate
...
- (void)initSubview
{
    ...
    [shareQR setTitle:@"编辑任务" forState:UIControlStateNormal];
    ...
    [keepQR setTitle:@"更换委派人" forState:UIControlStateNormal];
    ...
    [sweepQR setTitle:@"更换执行人" forState:UIControlStateNormal];
    ...
    UIButton *noticationBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    noticationBtn.frame = CGRectMake(0, thirdline.bottom , 120, 40);
    [noticationBtn setTitle:@"通知" forState:UIControlStateNormal];
    [noticationBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    noticationBtn.titleLabel.font = [UIFont systemFontOfSize:14];
    noticationBtn.tag = 4;
    [noticationBtn addTarget:self action:@selector(jumpConnect:) forControlEvents:UIControlEventTouchUpInside];
    [nextView addSubview:noticationBtn];
    
    nextView.height = noticationBtn.bottom;
...
//        [self.viewController.navigationController pushViewController:selectNewExcutePeopleVC animated:YES];
        ...
        [self.viewController.navigationController pushViewController:[[SelectExcutePeopleVC alloc] init] animated:YES];

    }
    else if (btn.tag == 4) {
        [self.viewController.navigationController pushViewController:[[TurnApplyVC alloc] init] animated:YES];
