...
//  XiaoYing
//
//  Created by chenchanghua on 16/11/8.
...
@interface CreateApplyVC () <UICollectionViewDelegateFlowLayout, UICollectionViewDataSource, UITableViewDataSource, UITableViewDelegate, UITextFieldDelegate, XYDropDownMenuDelegate,D3RecordDelegate,UITextViewDelegate,UIScrollViewDelegate>
{
...
- (void)setCategoryNameArray:(NSArray *)categoryNameArray
{
    ...
    if (categoryNameArray.count == 0) { //表示该身份下没有可用的申请
        [MBProgressHUD showMessage:@"该身份下没有可用的申请"];
    }
}
...
- (void)viewDidLoad {
    ...
    //刷新身份、类型、种类
    [self refreshDorpDownMenu];
}

//刷新身份、类型、种类
- (void)refreshDorpDownMenu
{
    //waitHUD
    ...
    [EmployeeViewModel getEmployeeMessageSuccess:^(NSArray *identityNameArray, NSArray *departmentIdArray, NSArray *departmentNameArray) {
        ...
        if (identityNameArray.count && departmentIdArray.count) {
            ...
            [ApplyViewModel memoryUserApplicationMessageWithDepartmentIds:departmentIdArray success:^(NSString *memoryPath) {
                ...
                //申请身份
                weakSelf.identityNameArray = identityNameArray;
                weakSelf.identityIdArray = departmentIdArray;
                
                NSArray *firstIdentity = [totalArray firstObject];
                if (firstIdentity.count > 0) {
                    //申请类型
                    ...
                    weakSelf.categoryNameArray = tempCategoryNameArray;
                    weakSelf.categoryIdArray = tempCategoryIdArray;
                    
                    NSDictionary *firstCategory = [firstIdentity firstObject];
                    NSArray *firstType = firstCategory[@"TypesList"];
                    if (firstType.count > 0) {
                        ...
                        weakSelf.typeNameArray = tempTypeNameArray;
                        weakSelf.typeIdArray = tempTypeIdArray;
                        weakSelf.typeTagArray = tempTypeTagArray;
                        
                    }else {
                        
                        weakSelf.typeNameArray = [[NSArray alloc] init];
                        weakSelf.typeIdArray = [[NSArray alloc] init];
                        weakSelf.typeTagArray = [[NSArray alloc] init];
                    }
                    
                }else {
                    
                    //申请类型
                    weakSelf.categoryNameArray = [[NSArray alloc] init];
                    weakSelf.categoryIdArray = [[NSArray alloc] init];
                    //申请种类
                    weakSelf.typeNameArray = [[NSArray alloc] init];
                    weakSelf.typeIdArray = [[NSArray alloc] init];
                    weakSelf.typeTagArray = [[NSArray alloc] init];
            ...
            //获取用户在该公司下的部门和职位失败
            [MBProgressHUD showMessage:@"没有可发送的申请，请联系相关管理员或前往综合管理进行设置。"];
...
- (void)submitAction:(UIButton *)btn
{
    ...
    if ([self.identityList.selectedString isEqualToString:@""]) {
        
        [MBProgressHUD showMessage:@"请选择身份"];
        return;
    }
    if ([self.categoryList.selectedString isEqualToString:@""]) {
        
        [MBProgressHUD showMessage:@"请选择类型"];
        return;
    }
    if ([self.typeList.selectedString isEqualToString:@""]) {
    ...
    //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    //检查现在所选择的身份、类型、种类是否‘过期’
    
    __weak typeof(self) weakSelf = self;
    [EmployeeViewModel getEmployeeMessageSuccess:^(NSArray *identityNameArray, NSArray *departmentIdArray, NSArray *departmentNameArray) {
        
        //身份的检验
        if([identityNameArray indexOfObject:weakSelf.identityList.selectedString] == NSNotFound){
            //身份已经过期
            [MBProgressHUD showMessage:@"申请信息已经变动，请重新选择"];
            [weakSelf refreshDorpDownMenu];
        }else {
            
            //类型的检测
            NSInteger identityNum = [identityNameArray indexOfObject:weakSelf.identityList.selectedString];
            [ApplyViewModel getUserApplicationCategoryWithDepartmentId:[departmentIdArray objectAtIndex:identityNum] success:^(NSArray *categoryNameArray, NSArray *cateoryIdArray) {
                
                if ([categoryNameArray indexOfObject:weakSelf.categoryList.selectedString] == NSNotFound) {
                    //类型已经过期
                    [MBProgressHUD showMessage:@"申请信息已经变动，请重新选择"];
                    [weakSelf refreshDorpDownMenu];
                }else {
                    ...
                    //种类的检测
                    NSInteger categoryNum = [categoryNameArray indexOfObject:weakSelf.categoryList.selectedString];
                    [ApplyViewModel getUserApplicationTypeWithDepartmentId:[departmentIdArray objectAtIndex:identityNum] categoryId:[cateoryIdArray objectAtIndex:categoryNum] success:^(NSArray *typeNameArray, NSArray *typeIdArray, NSArray *typeTagArray) {
                        
                        if ([typeNameArray indexOfObject:weakSelf.typeList.selectedString] == NSNotFound) {
                            //种类已经过期
                            [MBProgressHUD showMessage:@"申请信息已经变动，请重新选择"];
                            [weakSelf refreshDorpDownMenu];
                        }
    ...
    [ApplyViewModel createApplicationWithDepartmentId:[self.identityIdArray objectAtIndex:identityNum] TypeId:[self.typeIdArray objectAtIndex:typeNum] Tag:_moneyTextField.text Content:_tv.text PhotoIds:pictureIdsStr voiceIds:voidceIdsstr success:^(NSDictionary *dataList) {
        ...
        NSArray  *controlArray = weakSelf.navigationController.childViewControllers;
        ...
        if ([dataList[@"successful"] isEqual:@1]) {
...
- (void)setupViewContent{
    ...
    _scroll.frame = CGRectMake(0, 0, kScreen_Width, kScreen_Height - 64);
    ...
    [_backgroundView addSubview:_identityLab];
    
    // 身份选项
    [_backgroundView addSubview:self.identityList];
    ...
    // 类型选项
    [_backgroundView addSubview:self.categoryList];
    ...
    [_backgroundView addSubview:_kindLab];
    
    // 种类选项
    [_backgroundView addSubview:self.typeList];
    ...
    [_backgroundView addSubview:_moneyBackgroundView];
    ...
    // 确定scroll的大小
    _scroll.contentSize = CGSizeMake(kScreen_Width, self.pictureCollectonView.bottom+20);
...
{
    self.modelEssayCollectionView.origin = CGPointMake(0, self.pictureCollectonView.bottom+12);
    _backgroundView.height = self.modelEssayCollectionView.bottom;
    _scroll.contentSize = CGSizeMake(kScreen_Width, self.modelEssayCollectionView.bottom + 20);
}
...
- (void)keyBoardShow:(NSNotification *)obj
{
    if (_moneyTextField.isFirstResponder) {
        ...
        //得到其origin.y
        CGFloat yInSuperView = _moneyBackgroundView.frame.origin.y;
        
        //得到scrollView现在的y偏移量
        CGFloat offY = _scroll.contentOffset.y;
        
        //使其刚好位于导航栏的下方，计算距离差
        CGFloat y = yInSuperView - offY;
        
        //使scrollView移动
        _scroll.frame = CGRectMake(0, -y, kScreen_Width, kScreen_Height - 64 - y);
        ...
        //得到其origin.y
        CGFloat yInSuperView = _tv.frame.origin.y;
