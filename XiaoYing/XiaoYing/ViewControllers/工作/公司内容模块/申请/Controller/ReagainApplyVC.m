...
@interface ReagainApplyVC ()<UICollectionViewDelegateFlowLayout, UICollectionViewDataSource, UITableViewDataSource, UITableViewDelegate, UITextFieldDelegate, XYDropDownMenuDelegate,D3RecordDelegate,UITextViewDelegate,UIScrollViewDelegate>
{
...
- (XYDorpDownMenu *)identityList
{
    if (!_identityList) {
        ...
        //身份名称
        NSString *identityNameStr = [NSString stringWithFormat:@"%@-%@", self.applicationcreatorModel.departmentName, self.applicationcreatorModel.mastJobName];
        
        
        _identityList = [[XYDorpDownMenu alloc] initWithFrame:CGRectMake(58, 18, kScreen_Width - 65 - 15, 28) MenuTitle:identityNameStr DataSource:self.identityNameArray];
        _identityList.selectedString = identityNameStr;
...
- (XYDorpDownMenu *)categoryList
{
    if (!_categoryList) {
        ...
        //申请类型名称
        NSString *categoryNameStr = self.self.applicationcreatorModel.categoryName;
        
        _categoryList = [[XYDorpDownMenu alloc] initWithFrame:CGRectMake(58, (_identityLab.bottom +18), kScreen_Width - 65 - 15, 28) MenuTitle:categoryNameStr DataSource:self.categoryNameArray];
        _categoryList.selectedString = categoryNameStr;
...
- (XYDorpDownMenu *)typeList
{
    if (!_typeList) {
        ...
        //申请种类名称
        NSString *typeNameStr = self.applicationMessageModel.typeName;
        
        _typeList = [[XYDorpDownMenu alloc] initWithFrame:CGRectMake(58, (_typeLab.bottom +18), kScreen_Width - 65 - 15, 28) MenuTitle:typeNameStr DataSource:self.typeNameArray];
        _typeList.selectedString = typeNameStr;
...
- (void)submitAction:(UIButton *)btn
{
    ...
    NSInteger typeNum = [self.typeNameArray indexOfObject:self.typeList.selectedString];//在后面代码中兼容
    NSString *typeIdStr = (self.typeNameArray.count == 0)? self.applicationMessageModel.typeId : [self.typeIdArray objectAtIndex:typeNum];
    ...
    NSLog(@"self.typeList.selectedString~~%@", self.typeList.selectedString);
    
    NSLog(@"identityNum:~~%ld", identityNum);
    NSLog(@"typeNum:~~%ld", typeNum);
    NSLog(@"_moneyTextField.text:~~%@", _moneyTextField.text);
    NSLog(@"pictureIdsStr:~~%@", pictureIdsStr);
    NSLog(@"voidceIdsstr:~~%@", voidceIdsstr);
    ...
    [ApplyViewModel createApplicationWithDepartmentId:[self.identityIdArray objectAtIndex:identityNum] TypeId:typeIdStr Tag:_moneyTextField.text Content:_tv.text PhotoIds:pictureIdsStr voiceIds:voidceIdsstr success:^(NSDictionary *dataList) {
...
- (void)setupViewContent{
    ...
    NSString *applicationContentStr = self.applicationMessageModel.remark;//申请说明
...
{
    NSLog(@"menu.selectedString~~%@~~~%ld", menu.selectedString, indexPath.row);
