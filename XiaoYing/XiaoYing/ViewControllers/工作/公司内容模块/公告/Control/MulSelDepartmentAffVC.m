...
@interface MulSelDepartmentAffVC ()<UITableViewDataSource,UITableViewDelegate,UISearchBarDelegate>
{
    MBProgressHUD *hud;
...
@implementation MulSelDepartmentAffVC
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.model = model;
    BOOL isSame = NO;
    for (NSString *depaId in self.allowDepartments) {
        if ([model.DepartmentId isEqualToString:depaId]) {
            isSame = YES;
        }
    }
    if (isSame == NO) {
