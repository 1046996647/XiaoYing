...
@interface RenameFolderController ()<UITextFieldDelegate>

@property (nonatomic,strong) UITextField *field;

@end

@implementation RenameFolderController
...
- (void)setupViews
{
    ...
    field.text = self.currentFolderName;
...
- (void)btnAction:(UIButton *)btn
{
    if ([btn.titleLabel.text isEqualToString:@"确定"]) {
        ...
        if (self.field.text.length <= 0) {
            [MBProgressHUD showMessage:@"文件夹名称不能为空"];
        }else {
        
            [DocumentViewModel renameFolderWithOldFolderId:self.oldFolderId newFolderName:self.field.text success:^(NSDictionary *dataList) {
                
                NSLog(@"重命名文件夹成功:%@", dataList);
                [self.field resignFirstResponder];
                [self dismissViewControllerAnimated:YES completion:nil];
                
            } failed:^(NSError *error) {
                
                NSLog(@"重命名文件夹失败:%@", error);
            }];
