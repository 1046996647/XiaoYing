...
- (void)_setupCollectionView {
    ...
    bottomBar.frame = CGRectMake(0, kScreenHeight - 50, kScreenWidth, 50);
    __weak typeof(*&self) wSelf = self;
    ...
    [assetCell setWillChangeSelectedStateBlock:^BOOL(XMNAssetModel *asset) {
        ...
        if (!asset.selected) {
            ...
            if (asset.type == XMNAssetTypeVideo && self.selectedAssets.count > 0) {
                ...
                return YES;
            }else if ((self.selectedAssets.count + self.previousCount) >= photoPickerC.maxCount) {
                
                
                
                [self showAlertWithMessage:[NSString stringWithFormat:@"最多只能选择%ld张照片",self.selectedAssets.count]];
                return NO;
            }
            return YES;
        }else {
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (assetModel.type == XMNAssetTypeVideo) {
        ...
        previewC.previousCount = self.previousCount;
